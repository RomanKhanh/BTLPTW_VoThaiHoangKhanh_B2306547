...
function startEdit() {
  ...
  form.NgaySinh = toDisplayDate(auth.user?.NgaySinh);
...
<template>
  <div class="max-w-lg">
    ...
    >
      <div
        class="bg-gradient-to-r from-brand-600 to-brand-800 px-6 py-8 flex items-center gap-4"
      >
        <div
          class="h-16 w-16 rounded-2xl bg-white/20 flex items-center justify-center text-2xl font-bold text-white"
        >
        ...
        <div v-if="!editMode" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p class="text-sm font-medium text-ink-700 mt-0.5">
                {{ auth.user?.MaDocGia }}
              </p>
            </div>
            <div>
